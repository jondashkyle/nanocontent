...
function readPageSync (pathPage, opts) {
  ...
  return xtend(content, {
    ...
    url: slash(path.join('/', pathPage.replace(pathRoot, ''))),
    files: files,
    pages: pages
  })
  ...
  function getContent () {
    try {
      var contentFile = fs.readFileSync(path.join(pathPage, fileIndex), encoding)
      var contentParsed = utilContent.decode(contentFile)

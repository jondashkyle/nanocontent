  ...
  async function getFiles (files) {
    ...
    async function read (pathFile) {
      ...
      try {
        ...
        if (fileParsed.type === 'image') {
          var imageBuffer = await fs.readFile(path.join(pathPage, pathFile))
          var dimentions = sizeOf(imageBuffer)
          delete dimentions.type
          Object.assign(fileParsed, dimentions)
        }
